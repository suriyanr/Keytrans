...
#include "KeyTransHook.h" 


#include <stdio.h>

#if 1 // Turn it to 1 to turn debug on.
#define DEBUG 1
...
// Second byte = VK_Code
// This is exactly the same how Ligatures[even int][0 to 6] is stored.
...
int keytrans_index;
   ...
   // Lets read in the Ligature Rules. (max 10 as of now)
   index = 0;
   while (index < 20) {
   char str_rules[7];
   char *str_copy;
...
int count = 0;
   ...
   // We now see if there is a translation present in LigatureMap[][8];
   index = 0;
   while (1) {
